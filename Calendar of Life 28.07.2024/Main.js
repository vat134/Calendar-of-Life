document.addEventListener('DOMContentLoaded', () => {
    ...
    // Добавляем логику для вычисления пропущенных интервалов
    const lastVisitStr = localStorage.getItem('lastVisit');
    if (lastVisitStr) {
        const lastVisit = new Date(lastVisitStr);
        const now = new Date();
        
        // Приводим время к полуночи
        lastVisit.setHours(0, 0, 0, 0);
        now.setHours(0, 0, 0, 0);
        
        const missedIntervals = Math.floor((now - lastVisit) / 86400000); // 86400000 это 24 часа в миллисекундах
        squareCount += missedIntervals;
    }

    // Сохраняем текущее время как последнее посещение
    localStorage.setItem('lastVisit', new Date().toISOString());

    setInterval(() => {
        markCurrentDay();
        squareCount++;
        localStorage.setItem('squareCount', squareCount); // Сохраняем счетчик квадратов
        localStorage.setItem('lastVisit', new Date().toISOString()); // Обновляем время последнего посещения
    }, 86400000);
