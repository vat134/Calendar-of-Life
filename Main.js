...
function updateSquareColor(squareElement, value) {
    if (value.trim() === '-') {
        ...
        squareElement.style.boxShadow = '0 0 10px #8a8a8a'; // Свечение
    } else if (value.trim() !== '') {
        squareElement.classList.add('done');
        squareElement.classList.remove('missed');
        squareElement.style.animation = 'neonGreenToBlack 3s infinite'; // Введено значение
    } else {
        squareElement.classList.remove('done');
        squareElement.classList.add('missed');
        squareElement.style.backgroundColor = '#8a8a8a'; // Начальный цвет
        squareElement.style.boxShadow = '0 0 10px #8a8a8a'; // Свечение
    }
}
